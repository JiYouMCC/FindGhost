...
findghost.hall.updateMessageCallback(function(snapshot) {
    ...
    for (date in messages) {
        ...
        if (messageType == findghost.MESSAGE_TYPE.SYSTEM) {
            $("#messages").append($("<div></div>").addClass("text-danger").append($("<span></span>").text(("0" + dateTime.getHours()).slice(-2) + ":" + ("0" + dateTime.getMinutes()).slice(-2) + ":" + ("0" + dateTime.getSeconds()).slice(-2) + " ")).append($("<span></span>").text(message)));
        } else {
            var userDisplay = messages[date].displayName;
            $("#messages").append($("<div></div>").append($("<span></span>").text(("0" + dateTime.getHours()).slice(-2) + ":" + ("0" + dateTime.getMinutes()).slice(-2) + ":" + ("0" + dateTime.getSeconds()).slice(-2) + " ")).append($("<span></span>").text(userDisplay + "：")).append($("<span></span>").text(message)));
        }
    };
